...
# Onready Variables
@onready var model = $Ch14_nonPBR2
@onready var animation = $Ch14_nonPBR2/AnimationPlayer
...
func perform_jump():
	...
	jumpTween()
	animation.play("jump/jump")
	velocity.y = jump_force

func perform_flip_jump():
	AudioManager.jump_sfx.play()
	AudioManager.jump_sfx.pitch_scale = 0.8
	animation.play("filp/filp", -1, 2)
	velocity.y = jump_force
	await animation.animation_finished
	can_double_jump = false
	animation.play("jump/jump", 0.5)
...
func player_animations():
	...
	if is_on_floor():
		if is_moving(): # Checks if player is moving
			animation.play("walk/sneak_walk", 0.5)
			particle_trail.emitting = true
			footsteps.stream_paused = false
		else:
			animation.play("idle/mixamo_com", 0.5)
